...
struct Employee* createEmployee(char* last, char* first, char* title,
				char* nickname, int salary) {
...
char* getNickname(struct Employee* p) {
  return p ? p->nickname : "";
}
...
void setNickname(struct Employee* p, char* nickname){
  if (p != NULL)
    strcpy(p->nickname, nickname);
}
